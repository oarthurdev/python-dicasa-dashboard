...
def sync_company_data(company_id: str, config: dict):
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.error(
                    f"Error in sync loop for company {company_id}: {str(e)}")
                threading.Event().wait(60)  # Wait 1 minute before retry

    except Exception as e:
        logger.error(
            f"Fatal error in sync thread for company {company_id}: {str(e)}")
...
async def start_sync(company_id: str, background_tasks: BackgroundTasks):
    try:
        if company_id in sync_threads and sync_threads[company_id].is_alive():
            return {
                "status": "running",
                "message": f"Sync already running for company {company_id}"
            }

        config = supabase_client.client.table("kommo_config").select("*").eq(
        ...
        return {
            "status": "started",
            "message": f"Sync started for company {company_id}"
        }
...
async def get_sync_status(company_id: str):
    if company_id in sync_threads:
        return {
            "status":
            "running" if sync_threads[company_id].is_alive() else "stopped",
            "company_id": company_id
        }
...
async def stop_sync(company_id: str):
    if company_id in sync_threads:
        ...
        return {
            "status": "stopped",
            "message": f"Sync stopped for company {company_id}"
        }
    return {
        "status": "not_found",
        "message": f"No sync running for company {company_id}"
    }
